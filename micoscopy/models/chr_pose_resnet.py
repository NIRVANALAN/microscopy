...
__all__ = ['resnet101_chr_pose', 'resnet50_chr_pose']
...
class ResNetCHRPose(nn.Module):

    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None):
        super(ResNetCHRPose, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        ...
        self.deconv_bn = norm_layer(512)
        self.fc_pose = nn.Conv2d(512, 6, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        o3 = self.fc3(l2_7)
        #extract kpt
        kpt = self.deconv(l2_5)
        kpt = self.deconv_bn(kpt)
        kpt = F.relu(kpt)
        kpt = self.fc_pose(kpt)
        return o1,o2,o3,kpt


def resnet101_chr_pose(**kwargs):
    return ResNetCHRPose(Bottleneck, [3, 4, 23, 3], **kwargs)


def resnet50_chr_pose(**kwargs):
    return ResNetCHRPose(Bottleneck, [3, 4, 6, 3], **kwargs)
